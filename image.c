...
img_open(const fileinfo_t *file)
{
	...
	if (im == NULL && (file->flags & FF_WARN))
		error(0, 0, "%s: Error opening image (Imlib_Load_Error = %d)", file->name, err);
	return im;
}
...
CLEANUP void
img_close(img_t *img, int decache)
{
	int i;
...
void
img_check_pan(img_t *img, int moved)
{
	win_t *win;
	float w, h, ox, oy;
